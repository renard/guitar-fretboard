...
\newcommand{\mypitchline}[1]{
  \texttt{#1} &
  \getPitch{#1}{degree} &
  \getPitch{#1}{semitones} &
  \getPitch{#1}{tex} &
  \getPitch{#1}{tex small} &
  \texttt{\getPitch{#1}{reversed}} &
...
\newcommand{\myintervalline}[1]{
  \texttt{#1} &
  %% \texttt{\getInterval{#1}{aliases}} &
  \getInterval{#1}{degree} &
  \getInterval{#1}{semitones} &
  \getInterval{#1}{name} &
  %% \csname I#1\endcsname &
  \getInterval{#1}{tex} &
  \getInterval{#1}{tex small} &
  \getInterval{#1}{reversed} 
  \\
}
...
\begin{tcblisting}{doc-code,text above listing}
  \begin{fb}[frets before = 2, frets after = 2,
      legend = {Major scale (1, 2, 3, 4, 5, 6, 7)},
...
How this the \pC chord looks like when the guitar is open \pG tuned? Again
...
As you can see, setting the \meta{tuning} option update the whole
fretboard. This is very easy to create some new diagrams in different
tunings.

Now let's figure out how a left-handed guitarist can practice the \pC M7
arpeggio (and the whole major scale) in open \pG tuning:

\begin{tcblisting}{doc-code,text above listing}
  \begin{fb}[frets before = 2, frets after = 2,
      tuning = \tuning{guitar/open g}, left handed,
      legend = {\pC M7 arpeggio in open \pG tuning},
      fret numbers, scale=0.35]
...
\begin{tcblisting}{doc-code,text above listing}
  \begin{fb}[frets min=2, frets max=6,
    ...
    \note[highlight]{b2}
    \note[highlight, shade, limit={1}]{b2}
  \end{fb}
\end{tcblisting}
...
command or just when you need in the \refEnv{fb} environment or when using
the \refCom{note} command.
...
\begin{docKey}[tikz][]{legend position}{=style}{\{ shift = \{(0, -0.75)\}\}}
  Same as \refKey{/tikz/fret numbers position} but for the diagram legend.
...
\begin{docEnvironment}{fb}{\oarg{options}}
  ...
  \begin{docKey}[fb][]{split}{=boolean}{false}
    ...
    \refKey{/intervals/NOTE/reversed} is set). If note is a pitch both flat
  ...
  \begin{docKey}[fb][]{legend}{=text}{}
    If not blank, the \docValue{text} content is used for the diagram legend.
  \end{docKey}
...
\begin{docCommand}{note}{\oarg{options}\marg{id}}
  Place the note referenced by \marg{id} on the current fretboard. This
  command only makes sense in the \refEnv{fb} environment.

  You can override any parameter set in the \refCom{newInterval} command or
...
\begin{docCommand}{newInterval}{\oarg{options}\marg{id}}
  ...
  \begin{docKey}[intervals/NOTE][]{tex}{=tex}{}
  ...
  \begin{docKey}[intervals/NOTE][]{tex small}{=tex}{}
    Same as \refKey{/intervals/NOTE/tex} but used for fretboard limited
    space. Flats and sharps are smaller (\getInterval{CS}{tex}{} and
    \getInterval{CS}{tex small}).
  \end{docKey}

  \begin{docKey}[intervals/NOTE][]{semitones}{=0}{0}
    Semitone distance from the \pC note. This is used to determine the
    note's place on the fretboard.
  \end{docKey}

  \begin{docKey}[intervals/NOTE][]{reversed}{=note ref}{}
  ...
  the fretboard using the \refCom{note} command to override default
  behaviors.

  \begin{docKey}[intervals/NOTE][]{limit}{={list, of, strings}}{}
  ...
  \begin{docKey}[intervals/NOTE][]{style}{={style}}{}
    A complement style definition to \refKey{/tikz/note}.
  \end{docKey}

  \begin{docKey}[intervals/NOTE][]{highlight style}{={style}}{}
    A complement style definition to \refKey{/tikz/note highlight}.
  \end{docKey}

  \begin{docKey}[intervals/NOTE][]{split}{={boolean}}{false}
    Split that specific note if set to true.
  \end{docKey}

  \begin{docKey}[intervals/NOTE][]{shade}{={boolean}}{false}
    Shade that specific note if set to true.
  \end{docKey}

  \begin{docKey}[intervals/NOTE][]{highlight}{={boolean}}{false}
    Highlight that specific note if set to true.
  \end{docKey}

\end{docCommand}

\begin{docCommand}{copyInterval}{\oarg{options}\marg{src}\marg{to}}
  Copies the note \marg{src} to \marg{to} and override parameters given in
  \oarg{options}. See \refCom{newInterval}.
\end{docCommand}

\begin{docCommand}{getInterval}{\marg{id}\marg{key}}
...
\usepackage{guitar-fretboard}
\begin{document}
\begin{fb}[frets min=0, frets max=4, fret numbers,
    left handed, legend = {\pC}]%
  \note{C} \note{E} \note{G}
\end{fb}%
\end{document}
\end{example}
